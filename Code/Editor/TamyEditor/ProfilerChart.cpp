...
ProfilerChart::ProfilerChart( QWidget* parentWidget, uint timerId )
...
float ProfilerChart::addSample( float elapsedTime, float sampleValue )
{
   ...
   if ( refreshLines )
   {
      ...
      for ( int i = lastLineIdx; i >= 0; --i )
      {
         ...
         m_topValue = max( m_topValue, currNode->m_value );
