   ...
   : m_cascadeCalculationConfig( cascadesCount, cascadesPerRow, 3.0f )
{
   ...
   FilePath psPath( LIGHTING_SHADERS_DIR "Indexed/Directional/directionalLight.tpsh" );
   m_lightingShader = resMgr.create< PixelShader >( psPath, true );

   // load shadow shaders
   FilePath shadowDepthMapShaderPath( LIGHTING_SHADERS_DIR "Indexed/Directional/shadowDepthMapShader.tpsh" );
   m_shadowDepthMapShader = resMgr.create< PixelShader >( shadowDepthMapShaderPath, true );

   FilePath shadowProjectionPSPath( LIGHTING_SHADERS_DIR "Indexed/Directional/shadowProjectionShader.tpsh" );
