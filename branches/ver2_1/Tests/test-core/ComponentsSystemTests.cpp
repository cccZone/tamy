...
TEST(ComponentsManager, subscribingToServices)
{
   ...
   CPPUNIT_ASSERT_NO_THROW(compMgr.add(comp1));
   CPPUNIT_ASSERT_NO_THROW(compMgr.add(comp2));
...
TEST(ComponentsManager, invalidInitializationOrder)
{
   ComponentsManagerMock compMgr;
   ...
   CPPUNIT_ASSERT_EQUAL((unsigned int)0, compMgr.getComponentsCount());
}

///////////////////////////////////////////////////////////////////////////////

TEST(ComponentsManager, registeringSameServiceTwice)
{
   ComponentsManagerMock compMgr;

   MockComponentThatProvidesService* comp1 = new MockComponentThatProvidesService();
   MockComponentThatProvidesService* comp2 = new MockComponentThatProvidesService();

   CPPUNIT_ASSERT_NO_THROW(compMgr.add(comp1));
   CPPUNIT_ASSERT_EQUAL((unsigned int)1, compMgr.getComponentsCount());
